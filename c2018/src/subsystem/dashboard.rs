...
use crate::util::config::dashboard::*;
...
impl Handler for DashboardHandle {
    fn on_open(&mut self, _: Handshake) -> Result<()> {
        self.web_socket.timeout(PACKET_RATE, UPDATE)?;
        self.web_socket.timeout(PACKET_TIMEOUT, TIMEOUT)
    }
...
impl Subsystem<()> for Dashboard {
    fn run(&mut self) {
        let mut timeouts = 0;
        loop {
            connect(OPENMCT_URL, |web_socket| DashboardHandle {
                web_socket,
