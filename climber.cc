...
// Increments position of the climber. If the climber is currently resetting, the latch is set to true.
...
// Used when the climber is being used manually instead of automatically.
// When the climber is being used manually, no sensors are used. 
// Telemetry is a means of communication between the user and the machine.
...
void Climber::SetZeroingWind(Climber::Direction direction) {
    ...
    switch (direction) {
       // Climber is switching direction - brake is set to true (not enabled)
